...
if __name__ == "__main__":
    ...
    if st.button("Predict"):
        ...
        print("Result is:",max(result))

        st.bar_chart(data=result)
        sentiment_string = json.dumps({k: float(v) for k, v in result.items()})

        def max_sentiment(sentiment_string):
            sentiment_dict = json.loads(sentiment_string)
            max_value = max(sentiment_dict.values())
            max_key = max(sentiment_dict, key=sentiment_dict.get)
            return max_key, max_value

        max_key, max_value = max_sentiment(sentiment_string)
        
        st.write(f"The sentiment is {max_key} ({max_value*100})%")
